...
<span>
  ...
  v-for="department in orderedPost" :post="posts" :key="department.id">
  <router-link :to="{
        name: 'DepartmentPost',
        params: {
          ...
          id: department.id,
          content: department.content.rendered
        }
      }"
      v-html="department.title.rendered">
  </router-link>
  </li>
</span>
</template>
  ...
  export default {
    ...
    computed: {
      orderedPost: function () {
        return _.orderBy(this.departments, 'slug')
      },

      filteredPost: function () {
        var self = this;
        return _.orderBy(this.departments, 'slug').filter(function (post) {
          return _.includes(department.slug, self.searchText);
        })
      },
    },
    ...
    methods: {
      ...
      getMenu: function () { axios.get("http://athensadvisor.athensservices.com:81/wp-json/wp/v2/departments", {})
        .then(response => {
          var departments = [];
